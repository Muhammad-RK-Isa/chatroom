 ...
 *
 * Usage:
 *   node get-styles.mjs Button
 *   node get-styles.mjs Button Card Chip
 *
 * Output:
...
async function fetchGithubFallback(component) {
	...
	for (const path of patterns) {
		...
		try {
			...
			// Intentionally ignored — fallback patterns are tried sequentially
		}
	}
...
/**
 * Output a single styles result with formatted headers.
 */
function outputSingleStylesResult(result) {
...
/**
 * Output styles results, formatting single results or printing JSON for multiple.
 */
function outputStylesResults(results) {
	if (results.length === 1) {
		outputSingleStylesResult(results[0]);
	} else {
...
async function main() {
	...
	if (args.length === 0) {
		console.error("Usage: node get-styles.mjs <Component1> [Component2] ...");
		console.error("Example: node get-styles.mjs Button Card");
		process.exit(1);
	}
	...
	if (data?.results) {
		...
		outputStylesResults(data.results);
