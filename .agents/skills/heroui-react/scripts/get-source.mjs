 ...
 *
 * Usage:
 *   node get-source.mjs Button
 *   node get-source.mjs Button Accordion Card
 *
 * Output:
...
async function fetchGithubFallback(component) {
	...
	for (const path of patterns) {
		...
		try {
			...
			// Silently continue to next pattern on fetch failure
		}
	}
...
/**
 * Output a single source result to stdout.
 */
function outputSingleSourceResult(result) {
...
/**
 * Output source results to stdout (single or multiple).
 */
function outputSourceResults(results) {
	if (results.length === 1) {
		outputSingleSourceResult(results[0]);
	} else {
...
async function main() {
	...
	if (args.length === 0) {
		console.error("Usage: node get-source.mjs <Component1> [Component2] ...");
		console.error("Example: node get-source.mjs Button Accordion");
		process.exit(1);
	}
	...
	if (data?.results) {
		...
		outputSourceResults(data.results);
		return;
	}
